...
export async function generateMetadata({
  ...
  const member = /^\d+$/.test(id) ? await getFacultyById(Number(id), "en") : null;
  if (!member) notFound();
...
/** 見 app/(site)/faculty/[id]/page.tsx —— 這是它的英文孿生。 */
export default async function Page({
  ...
  const member = await getFacultyById(numeric, "en");
  if (!member) notFound();

  return <FacultyProfile lang="en" member={member} />;
}
