...
export async function generateStaticParams() {
  const counts = await Promise.all(
    NEWS_CATEGORIES.map(async (c) => ({
      slug: c.slug,
      totalPages: (await getNewsPage(1, "en", c.category)).totalPages,
    }))
  );
...
export async function generateMetadata({
  ...
  return listingMetadata(`/news/category/${slug}`, "en", copy?.title.en ?? "News");
}
...
export default async function Page({
  ...
  return <NewsRoute lang="en" category={category} page={n} />;
}
