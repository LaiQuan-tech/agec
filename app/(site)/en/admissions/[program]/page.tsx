...
export async function generateMetadata({
  ...
  return articleMetadata(`/admissions/${slug}`, "en", {
    title: `${loaded.program.name}${ADMISSIONS.programPage.titleSuffix.en}`,
    excerpt: copy?.tagline.en ?? loaded.program.description,
    cover_url: null,
  });
}
...
export default async function Page({
  ...
  const [notices, documents, links] = await Promise.all([
    getAdmissionsNews(nameZh, "en"),
    getDocumentsForProgram("admissions", nameZh, "en"),
    getLinksForProgram("admissions", nameZh, "en"),
  ]);
