...
- `GivingPage.tsx` + `CopyButton.tsx`（client）— 匯款帳號資訊 `/alumni/giving`（/alumni#section-3「支持農經」那顆「前往支持農經」按鈕與頁尾同名連結的落點）。值全走 `page_copy`（page=`giving`，`lib/page-copy/giving.ts`，後台 /admin/giving），**沒有字典可退、預設全空**；帳號沒填時整頁只印「整理中」（按鈕照印）。內頁只有一顆「複製全部匯款資訊」，每欄不各放複製鈕（客戶要求）。`CopyButton` 是站上唯一碰剪貼簿的元件：`navigator.clipboard` 失敗退回 `execCommand("copy")`，兩條路都失敗**不印「已複製」**
