 ...
 *
 * 🔴 **標籤必須與落點區塊的小標一字不差** —— 不是「出現在小標或大標其中之一」
 * 就好。點「專任師資」落在一個小標寫著「師資陣容」的區塊，即使右邊的大標
 * 確實寫著「專任師資」，同一段還是出現了三種說法，讀者仍然會覺得對不上
 * （2026-09 客戶連續兩次回報的就是這件事）。
 *
 * 代價是十三個區塊的小標會與同一行右側的大標重複，那是已知且被接受的取捨。
 * 完整規則、被推翻的舊做法與唯一的例外，都寫在 lib/i18n/eyebrows.ts 的檔頭。
 *
 * /sitemap 也是印同一組標籤，所以對不上的話會錯兩個地方。
 */
